...
pub fn parse_modifiers(mods: Option<&String>) -> String {
    let mess = match mods {
        Some(v) => {
...
impl Layer {
    pub fn from(key_map: &crate::KeyMap, is_desktop: bool) -> Layer {
        if is_desktop {
            Layer::Desktop(DesktopLayer::from(key_map))
        } else {
            Layer::Mobile(MobileLayer::from(key_map))
        }
    }
}

// Keyboard[is_mobile] -> KeyMap[modifiers; ] -> Keys[to, iso, long press]
// Keyboard -> Mode<ModifiersKey, DesktopLayer>
#[derive(Debug, Clone)]
pub struct DesktopLayer {
    pub mode: String,
    pub keys: BTreeMap<IsoKey, String>,
}
...
impl DesktopLayer {
    fn new(mode: String, keys: BTreeMap<IsoKey, String>) -> DesktopLayer {
...
impl MobileLayer {
    fn new(mode: String, keys: Vec<Vec<String>>) -> MobileLayer {
...
impl<'a> Iterator for DesktopLayerIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let key = match IsoKey::from_str(&iso_k) {
            Ok(v) => v,
            Err(_) => return None
        };
...
impl From<&crate::KeyMap> for DesktopLayer {
    fn from(key_map: &crate::KeyMap) -> DesktopLayer {
        use std::str::FromStr;

        let mut keys = BTreeMap::new();

        for map in key_map.keys.iter() {
            if let Ok(key) = IsoKey::from_str(&map.iso) {
...
impl From<&crate::KeyMap> for MobileLayer {
    fn from(key_map: &crate::KeyMap) -> MobileLayer {
        let mut keys = vec![];
        let mut next = vec![];
...
impl From<&MobileLayer> for String {
    fn from(layer: &MobileLayer) -> String {
        layer.keys.iter().map(|row| row.join(" ")).collect::<Vec<_>>().join("\n")
    }
}
